...
\begin{align} 
 \label{eq:truncated_alpha_MaL}
  & \mathcal{L}( \boldsymbol x |  x_{\min},  l_\alpha, u_\alpha) = \int_{l_\alpha}^{u_\alpha} \frac{1}{(u_\alpha - l_\alpha)} \frac{1}{(\alpha - 1)} \left( \frac{\alpha - 1}{x_{\min} } \right)^N \left(\frac{ \prod_{i= 1}^N x_i}{ x_{\min}^N }\right)^{-\alpha} d\alpha, \\ %TODO: maybe remove this as redundant (just state 
